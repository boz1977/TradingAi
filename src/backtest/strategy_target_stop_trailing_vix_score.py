...
# PARAMETRI STRATEGIA
# =========================
STRATEGY_NAME = "target_stop_trailing_vix_score"
...
ENTRY_SCORE_THRESHOLD = 4


def prepare_data(df: pd.DataFrame) -> pd.DataFrame:
    """
    Prepara il dataset e calcola feature base + feature score.
    """
    df = df.copy()
    ...
    df["ma20"] = df.groupby("ticker")["Adj Close"].transform(
    ...
    df["ma100"] = df.groupby("ticker")["Adj Close"].transform(
    ...
    df["ma50_slope"] = df.groupby("ticker")["ma50"].transform(
        lambda x: x.diff(5)
    )

    df["distance_from_ma50"] = (df["Adj Close"] / df["ma50"]) - 1

    df["volatility_20"] = df.groupby("ticker")["return"].transform(
    ...
    df["entry_score"] = df.apply(calculate_entry_score, axis=1)

    return df


def calculate_entry_score(row: pd.Series) -> int:
    """
    Score ingresso da 0 a 7.
    """
    score = 0

    if pd.notna(row["ma50"]) and row["Adj Close"] > row["ma50"]:
        score += 1

    if pd.notna(row["ma20"]) and pd.notna(row["ma50"]) and row["ma20"] > row["ma50"]:
        score += 1

    if pd.notna(row["ma50_slope"]) and row["ma50_slope"] > 0:
        score += 1

    if pd.notna(row["rsi"]) and row["rsi"] < 60:
        score += 1

    if "VIX" in row.index and pd.notna(row["VIX"]) and row["VIX"] < 18:
        score += 1

    if "vix_ma10" in row.index and pd.notna(row["VIX"]) and pd.notna(row["vix_ma10"]) and row["VIX"] < row["vix_ma10"]:
        score += 1

    if "sp500_above_ma50" in row.index and row["sp500_above_ma50"] == 1:
        score += 1
...
def entry_condition(row: pd.Series) -> bool:
    """
    Ingresso score-based con gate VIX obbligatorio.
    """
...
def apply_strategy(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for ticker, group in df.groupby("ticker"):
        print(f"Esecuzione strategia score su {ticker}...")
...
def build_summary(df: pd.DataFrame, trade_log: pd.DataFrame) -> pd.DataFrame:
    ...
    if not trade_log.empty:
        ...
        for ticker, trades_group in trade_log.groupby("ticker"):
            ...
            avg_trade_return_pct = trades_group["net_return_pct"].mean()
            win_rate_pct = (trades_group["net_return_pct"] > 0).mean() * 100 if closed_trades > 0 else np.nan
            avg_entry_vix = trades_group["entry_vix"].mean()
            avg_exit_vix = trades_group["exit_vix"].mean()
            avg_holding_days = trades_group["holding_days"].mean()
            ...
            num_take_profit = int(exit_reason_counts.get("take_profit", 0))
...
def build_entries(df: pd.DataFrame) -> pd.DataFrame:
    cols = ["Date", "ticker", "Adj Close", "ma20", "ma50", "rsi", "entry_score"]
    if "VIX" in df.columns:
        cols.append("VIX")
