...
# PARAMETRI STRATEGIA
# =========================
STRATEGY_NAME = "target_stop_trailing_vix_regime"
...
def run_strategy_for_ticker(group: pd.DataFrame) -> pd.DataFrame:
    ...
    for i in range(len(group)):
        ...
        if not in_position:
            ...
            # Exit di regime VIX
            if USE_VIX_FILTER and "VIX" in group.columns:
    ...
    # Posizione reale dal giorno successivo
...
def build_trade_log(df: pd.DataFrame) -> pd.DataFrame:
    """
    Costruisce un trade log robusto basato sugli eventi espliciti.
    """
    records = []
